...
Click Add project → name it cipher-html (or anything you want)
Disable Google Analytics (not needed) → click Create project
1a — Enable Email/Password Authentication
In your Firebase console, go to Authentication → Get started
...
Go to Project Settings (gear icon) → Your apps tab
Click the Web icon (</>)
Register the app (name it anything, don't enable Hosting)
...
Replace ALL existing rules with the contents of firestore.rules in this repo
Click Publish
Step 4 — Create Your Admin Account
Before anyone else, you create your own account:

Open your GitHub Pages URL (see Step 6) or run the project locally
Go to register.html — but you need an invite first
Go directly to Firebase Console → Firestore → invites collection
Manually add a document with these fields:
code:     "ADMIN-INIT"
username: "your_admin_handle"    ← must match ADMIN_USERNAME in config
used:     false
Visit register.html?invite=ADMIN-INIT
Set your password and register
Back in Firestore → users collection → find your new user document
Manually set the isAdmin field to true
Log in at index.html — you will be redirected to admin.html
Step 5 — Invite Friends
Once logged in as admin:

Go to admin.html
Type your friend's desired username → click GENERATE
Copy the invite code or the full registration link
Send it to them — they visit the link, set their password, and they're in
Step 6 — Deploy to GitHub Pages
Option A: GitHub.com (easiest)
Create a new public repository on GitHub
Drag and drop the entire cipher-html/ folder contents into the repo
Go to Settings → Pages
Under Source, select main branch, /root folder → Save
